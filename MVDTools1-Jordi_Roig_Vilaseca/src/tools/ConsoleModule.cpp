...
void ConsoleModule::init()
{
	...
	commands_.push_back("savepoint");
	commands_.push_back("gotopoint");
...
void ConsoleModule::BuildCommand(const char* cmd)
{
	...
	*/
	...
	if (input.find("savepoint") != std::string::npos) {
		com_found = true;

		SavedPointNames.push_back(v[1].c_str());
		
		Transform& trans = ECS.getComponentFromEntity<Transform>(ECS.getEntity("PlayerFPS"));
		lm::vec3 pos = trans.position();

		SavedPointPositions.push_back(pos);
		ConsoleWrite(false, "Position saved");
	}

	if (input.find("gotopoint") != std::string::npos) {
		com_found = true;
		bool pos_found = false;

		std::cout << v[1];
		for (int i = 0; i < SavedPointNames.size(); i++) {
			
			if (strcmp(SavedPointNames[i].c_str(), v[1].c_str()) == 0) {
				

				Transform& trans = ECS.getComponentFromEntity<Transform>(ECS.getEntity("PlayerFPS"));
				trans.translate(trans.position()*-1);
				trans.translate(SavedPointPositions[i]);
		...
		if (pos_found) {
			ConsoleWrite(false, "Going to point...");
		}
		else {
			ConsoleWrite(false, "Point not found!");
		}
